...
const TABLE_BINDINGS = [
  ['dealTeamTable', 'Deal Team'],
  ['briefSnapshotsTable', 'Brief Snapshots'],
  ['sourceRecordsTable', 'Source Records'],
  ['dealsTable', 'Deals'],
  ['calendarTable', 'Calendar'],
] as const;

type TableBindingKey = (typeof TABLE_BINDINGS)[number][0];

/** Explicit bindings grant the extension access to the selected tables. */
function getCustomProperties(base: ReturnType<typeof useBase>) {
  return TABLE_BINDINGS.map(([key, label]) => ({
    key,
    label,
    type: 'table' as const,
...
type ComputerStation = {
  ...
  /** Table chosen in the extension settings, shared with the Kanban element. */
  readonly tableKey: TableBindingKey;
...
function ComputerRecordBrowser({
  ...
  const primaryField = table?.fields.find((field) => field.isPrimaryField) ?? null;
  const detailFields = useMemo(
    () => table?.fields.filter((field) => field.id !== primaryField?.id) ?? [],
    [table, primaryField]
  );
  const subtitleFields = useMemo(() => detailFields.slice(0, 2), [detailFields]);
...
function SourceRecordsReview({
  ...
  // Never infer a writable status field from an unrelated single-select.
  const reviewField = useMemo(
    () =>
      table?.fields.find(
        (field) =>
          field.name.trim().toLowerCase() === 'review status' &&
          String(field.type) === 'singleSelect'
      ) ?? null,
    [table]
  );
  ...
  const acceptChoice = useMemo(
    () =>
      reviewChoices.find((choice) => choice.name.trim().toLowerCase() === 'accepted') ??
      reviewChoices.find((choice) => choice.name.trim().toLowerCase() === 'approved') ??
      null,
    [reviewChoices]
  );

  const isAccepted = (rec: unknown) =>
    ['accepted', 'approved'].includes(safeRead(rec, reviewField).trim().toLowerCase());
  ...
  const setStatus = useCallback(
    async (recordId: string, choiceId: string) => {
      if (
        ...
        !reviewChoices.some((choice) => choice.id === choiceId)
      ) return false;
    ...
    [table, reviewField, canUpdate, reviewChoices]
  );
      ...
      if (openRecord) {
        ...
        if (e.key === 'Enter' && e.shiftKey) {
          ...
          if (acceptChoice && canUpdate && !saving) {
            ...
            void setStatus(recordId, acceptChoice.id).then((saved) => {
              if (saved) {
            ...
            >
              ...
              >
                ...
                {!reviewField ? (
                  ...
                  >
                    ADD A SINGLE-SELECT FIELD NAMED REVIEW STATUS TO UPDATE REVIEWS.
                  </div>
                ) : (
...
function PokeMartInterface() {
  ...
  const boundTables = Object.fromEntries(
    TABLE_BINDINGS.map(([key]) => [key, customPropertyValueByKey[key] ?? null])
  ) as Record<TableBindingKey, ComputerTable>;
  const table = boundTables.dealTeamTable;
  const briefSnapshotsTable = boundTables.briefSnapshotsTable;
  const sourceRecordsTable = boundTables.sourceRecordsTable;
  ...
  const dealsRecords = useRecords(boundTables.dealsTable ?? null);
  const calendarRecords = useRecords(boundTables.calendarTable ?? null);
  const recordsByTable: Record<TableBindingKey, ComputerRecords> = {
    dealTeamTable: records,
    briefSnapshotsTable: briefSnapshotsRecords,
    sourceRecordsTable: sourceRecordsRecords,
    dealsTable: dealsRecords,
    calendarTable: calendarRecords,
  };
  const missingTables = TABLE_BINDINGS.filter(
    ([key]) =>
      ['dealTeamTable', 'briefSnapshotsTable', 'sourceRecordsTable'].includes(key) &&
      !boundTables[key]
  ).map(([, label]) => label);
  ...
  if (errorState || missingTables.length > 0) {
    return (
      <div
        className="w-full h-full flex items-center justify-center p-6"
        style={{ backgroundColor: PK.sky, color: PK.ink, fontFamily: 'monospace' }}
        role="status"
      >
        <div>
          <h2>Connect the Poké Mart tables</h2>
          <p>
            Open this element's settings and select the same tables used by Poké Kanban.
          </p>
          {missingTables.length > 0 && <p>Required: {missingTables.join(', ')}.</p>}
          {errorState && <p>The table settings could not be loaded. Reopen the settings and check the selected tables.</p>}
          <p>Deals and Calendar are optional while their terminals are decorative.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      {openBrowser && openBrowser.key !== 'source-records' && (
        <ComputerRecordBrowser
          station={openBrowser}
          table={boundTables[openBrowser.tableKey]}
          records={recordsByTable[openBrowser.tableKey]}
